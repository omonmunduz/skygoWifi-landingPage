...
const IndexPage = () => {
  ...
  return (
    <Layout>
      ...
      <div className="w-screen flex flex-wrap bg-indigo-900 p-5 ">
        <div className="w-full flex md:justify-end md:w-9/12 md:m-auto  lg:w-2/4 p-10">
          <animated.div style={ {transform: props.xy.to(trans4)}} className="w-screen lg:w-3/4">
            <h1 className="text-4xl text-white md:text-5xl lg:text-6xl">SKYGO <span className="text-yellow-200">Portable WiFi</span></h1>
    ...
    Get Started
        ...
        <div className="w-full md:w-9/12 md:m-auto flex md:justify-start lg:w-2/4 p-5" onMouseMove = {( {clientX: x, clientY: y}) => set({ xy: calc(x, y)})}>
          <animated.div style={ {transform: props.xy.to(trans1)}} className="relative z-10">
              <StaticImage
